...
def char_maps(text: str):
    ...
    char_to_idx = {char: idx for idx, char in enumerate(unique)}
    idx_to_char = {idx: char for idx, char in enumerate(unique)}
    # ========================
...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    idx_tensor = torch.tensor([char_to_idx[c] for c in list(text)]).view(N,1)
    result = result.scatter_(1, idx_tensor, 1)
    # ========================
    return result
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    text_idx = torch.argmax(embedded_text, dim=1)
    result = [idx_to_char[i.item()] for i in text_idx]
    result = ''.join(result)
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    samples = chars_to_onehot(text, char_to_idx)[: N*seq_len] \
                .view(-1, seq_len, len(char_to_idx)) \
                .to(device)
    
    embbeded_labels = chars_to_onehot(text, char_to_idx)[1 : N*seq_len+1] \
                        .view(-1, seq_len, len(char_to_idx)) \
                        .to(device)
    labels = torch.argmax(embbeded_labels, dim=2)
